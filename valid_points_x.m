...
t = 4;% we have 5 points in totoal now
ft = fire{t}';
clear all_b;
all_b = [];
...
for i=1:size(ft,2)
 ...
 %       [b,bint1,r1,rint1,s] = regress(nowfiring,[xf1,f2-mean(f2)]);
   ...
   %     regfact(valued_num,: ) = s(:,1);
        ...
        c1 = b(2)^2*s11+b(3)^2*s22+2*b(2)*b(3)*s12-K2;
        x1 = roots([a1,b1,c1]); 
  ...
  %       plot([min(x1),max(x1)],[0,0],'--gs','LineWidth',2,'MarkerSize',10,'MarkerEdgeColor','b','MarkerFaceColor',[0.5,0.5,0.5]);
   %     hold on;
        ...
        c2 = b(3)*b(3)*s22+2*b(2)*b(3)*s12 + b(2)*b(2)*s11-K1;
        x2 = roots([a2,b2,c2]);
  ...
  %        plot([min(x2),max(x2)],[1,1],'--gs','LineWidth',2,'MarkerSize',10,'MarkerEdgeColor','b','MarkerFaceColor',[0.5,0.5,0.5]);
        ...
        c3 = b(3)*b(3)*s22+ 2*b(2)*b(3)*s12 + b(2)*b(2)*s11-K2;
        x3 = roots([a3,b3,c3]);
...
%        hold off;
        ...
        if(max(x2)<max(x1)&& min(x1)<min(x2))
            if(min(x3)>max(x2) || max(x3)<min(x2))
